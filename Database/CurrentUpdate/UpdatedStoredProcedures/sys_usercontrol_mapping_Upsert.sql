...
DROP PROCEDURE IF EXISTS `sys_usercontrol_mapping_Upsert`$$
...
BEGIN
    ...
    BEGIN
        SET p_Status = 1;
        SET p_ErrorMsg = 'Error upserting user control mapping';
        ROLLBACK;
    END;
    ...
    WHERE WindowFormMappingID = p_WindowFormMappingID
      AND CodebaseName = p_CodebaseName
    LIMIT 1;

    IF v_ExistingID IS NOT NULL THEN
        -- Update existing
        UPDATE UserControlMapping
    ...
    ELSE
        ...
        VALUES (p_WindowFormMappingID, p_CodebaseName, p_UserFriendlyName, p_IsActive);

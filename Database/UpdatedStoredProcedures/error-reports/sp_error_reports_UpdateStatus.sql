...
DELIMITER ;;
CREATE DEFINER=`root`@`172.16.1.104` PROCEDURE `sp_error_reports_UpdateStatus`(IN `p_ReportID` INT, IN `p_NewStatus` VARCHAR(20), IN `p_DeveloperNotes` TEXT, IN `p_ReviewedBy` VARCHAR(100), IN `p_ReviewedDate` DATETIME, OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
proc:BEGIN

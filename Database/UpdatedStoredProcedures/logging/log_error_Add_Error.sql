...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `log_error_Add_Error`(IN `p_User` VARCHAR(100), IN `p_Severity` VARCHAR(50), IN `p_ErrorType` VARCHAR(100), IN `p_ErrorMessage` TEXT, IN `p_StackTrace` TEXT, IN `p_ModuleName` VARCHAR(100), IN `p_MethodName` VARCHAR(100), IN `p_AdditionalInfo` TEXT, IN `p_MachineName` VARCHAR(100), IN `p_OSVersion` VARCHAR(100), IN `p_AppVersion` VARCHAR(50), IN `p_ErrorTime` DATETIME, OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN

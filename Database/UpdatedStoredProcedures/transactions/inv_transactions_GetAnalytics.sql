...
DELIMITER ;;
CREATE DEFINER=`root`@`172.16.1.104` PROCEDURE `inv_transactions_GetAnalytics`(IN `p_UserName` VARCHAR(100), IN `p_IsAdmin` BOOLEAN, IN `p_FromDate` DATETIME, IN `p_ToDate` DATETIME, OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN

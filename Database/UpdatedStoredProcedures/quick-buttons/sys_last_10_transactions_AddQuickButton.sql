...
DELIMITER ;;
CREATE DEFINER=`root`@`172.16.1.104` PROCEDURE `sys_last_10_transactions_AddQuickButton`(IN `p_User` VARCHAR(255), IN `p_PartID` VARCHAR(255), IN `p_Operation` VARCHAR(255), IN `p_Quantity` INT, IN `p_Position` INT, OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN

/*!50003 DROP PROCEDURE IF EXISTS `sys_last_10_transactions_Insert` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`172.16.1.104` PROCEDURE `sys_last_10_transactions_Insert`(IN `p_User` VARCHAR(255), IN `p_PartID` VARCHAR(255), IN `p_Operation` VARCHAR(255), IN `p_Quantity` INT, IN `p_Position` INT, OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN
    ...
    ELSE
        ...
        -- Simple INSERT without shifting - used by cleanup to rebuild button list
        ...
        IF v_RowCount > 0 THEN
            ...
            SET p_ErrorMsg = CONCAT('Quick button inserted at position ', p_Position);

        ELSE

            SET p_Status = -3;

            SET p_ErrorMsg = 'Failed to insert quick button';

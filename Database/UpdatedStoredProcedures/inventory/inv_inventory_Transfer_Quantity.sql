...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `inv_inventory_Transfer_Quantity`(IN `p_BatchNumber` VARCHAR(255), IN `p_PartID` VARCHAR(255), IN `p_Operation` VARCHAR(255), IN `p_TransferQuantity` INT, IN `p_OriginalQuantity` INT, IN `p_NewLocation` VARCHAR(255), IN `p_User` VARCHAR(255), OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN

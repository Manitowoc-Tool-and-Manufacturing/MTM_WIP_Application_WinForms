...
DELIMITER ;;
CREATE DEFINER=`root`@`172.16.1.104` PROCEDURE `sys_parameter_prefix_overrides_Add`(IN `p_ProcedureName` VARCHAR(128), IN `p_ParameterName` VARCHAR(128), IN `p_OverridePrefix` VARCHAR(10), IN `p_Reason` VARCHAR(500), IN `p_CreatedBy` VARCHAR(50), OUT `p_OverrideId` INT, OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN

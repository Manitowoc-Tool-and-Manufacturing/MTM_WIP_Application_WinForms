...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sys_parameter_prefix_overrides_Update_ById`(IN `p_OverrideId` INT, IN `p_ProcedureName` VARCHAR(128), IN `p_ParameterName` VARCHAR(128), IN `p_OverridePrefix` VARCHAR(10), IN `p_Reason` VARCHAR(500), IN `p_ModifiedBy` VARCHAR(50), OUT `p_Status` INT, OUT `p_ErrorMsg` VARCHAR(500))
BEGIN

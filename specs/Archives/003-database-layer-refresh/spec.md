...
**Implementation**: 100% accurate parameter prefix information by querying database schema directly. One-time startup cost (~100-200ms) to query all procedures and cache results. Handles all prefix types (p_, in_, o_) automatically. Fallback to convention (p_ default, in_ for Transfer*/transaction* procedures) if schema query fails.
...
3. **C# Model Property Alignment**
   ...
   - **When**: Comparing parameter names to model properties (removing p_ prefix)
...
- **Behavior**: Falls back to convention-based prefix detection (p_ default, in_ for Transfer*/transaction* procedures), logs warning "Parameter cache initialization failed, using fallback conventions", application continues with reduced accuracy
...
- Detect prefix per parameter (p_, in_, o_, or none)
- Apply detected prefix when executing stored procedures
- Fall back to convention (p_ default, in_ for Transfer*/transaction*) if schema query fails
...
- **Fallback**: Uses convention (p_ default, in_ for Transfer*/transaction*) if query fails
...
- **Current State**: Parameter prefix errors occur regularly (p_ vs in_ mismatch)

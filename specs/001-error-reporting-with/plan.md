...
| V. Environment-Aware Database Selection | ✅ PASS | Uses existing `Helper_Database_Variables.GetConnectionString()` for database connectivity. Debug builds target `mtm_wip_application_winforms_test`, Release builds target `mtm_wip_application`. No hardcoded connection strings. |
...
├── Forms/
│   └── ErrorDialog/
│       ├── Form_ReportIssue.cs              # NEW: Report Issue dialog
│       └── Form_ReportIssue.Designer.cs     # NEW: Designer code
│
├── Data/
│   └── Dao_ErrorReports.cs                  # NEW: Error reports data access
│
├── Services/
│   ├── Service_ErrorReportQueue.cs          # NEW: Offline queue management
...
│           └── sp_error_reports_Insert.sql  # NEW: Insert stored procedure
│
├── Controls/
│   └── SettingsForm/
│       └── Control_DeveloperSettings.cs     # MODIFIED: Add sync menu item
│
└── Program.cs                               # MODIFIED: Register startup sync
```
...
4. File move failure recovery → Idempotent execution with retry
5. Queue directory location → %APPDATA%\MTM_Application\ErrorReports\
6. Corrupted file handling → .corrupt extension with isolation
7. Retention and cleanup → 30-day configurable retention with startup cleanup

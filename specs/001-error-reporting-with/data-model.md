...
### Model_QueuedErrorReport (C# Class)
...
namespace MTM_WIP_Application_WinForms.Models
{
    ...
    public class Model_QueuedErrorReport
    {
        ...
        /// <summary>
        /// Creates a Model_QueuedErrorReport from a FileInfo object.
        /// </summary>
        public static Model_QueuedErrorReport FromFileInfo(FileInfo fileInfo)
        {
            return new Model_QueuedErrorReport
            {
...
└─────────────────────────────────────────┘
             ...
             │
┌────────────┴────────────────────────────┐
│  Model_ErrorReport (C# Application)     │
└─────────────────────────────────────────┘
             ↑
...
Check database connectivity
   ...
   [ONLINE] → Dao_ErrorReports.InsertReportAsync()
       ...
       ↓            ↓
       ↓     DaoResult<int>.Success(reportID, message)
       ↓
...
Check database connectivity
   ...
   [OFFLINE] → Service_ErrorReportQueue.QueueReportAsync()
       ...
       ↓            ↓
       ↓     Return DaoResult<string>.Success(filePath, "Queued")
       ↓
...
Check database connectivity
   ...
   [ONLINE] → Enumerate files in Pending folder
       ...
       ↓            ↓
       ↓     DaoResult<int>.Success(successCount, message)
       ↓
...
- **C# model classes** (Model_ErrorReport, Model_QueuedErrorReport) for application layer

...
const tools: Tool[] = [
  ...
  {
    name: "validate_ui_scaling",
    description:
      "Validate WinForms UI files for DPI scaling and resolution independence. Checks for: AutoScaleMode.Dpi, Core_Themes.ApplyDpiScaling calls, proper font sizing, adequate button sizes, DataGridView AutoSizeColumnsMode, and responsive layouts. Returns compliance report with critical/error/warning issues.",
    inputSchema: {
      type: "object",
      properties: {
        source_dir: {
          type: "string",
          description: "Absolute path to source code directory (Forms/ or Controls/)",
        },
        recursive: {
          type: "boolean",
          description: "Whether to search subdirectories recursively",
        ...
        file_types: {
          type: "array",
          items: { type: "string" },
          description: "File patterns to scan (default: ['*.cs', '*.Designer.cs'])",
          default: ["*.cs", "*.Designer.cs"],
        },
      },
      required: ["source_dir"],
    },
  ...
  {
    name: "generate_ui_fix_plan",
    description:
      "Generate comprehensive JSON fix plan from UI validation results. Analyzes UI files for DPI/scaling issues and creates structured fix actions with line numbers, current code, recommended fixes, and validation requirements. Output JSON can be used by auto-fix tools with built-in backup and corruption detection support.",
    inputSchema: {
      type: "object",
      properties: {
        source_dir: {
          type: "string",
          description: "Absolute path to source directory (Forms/ or Controls/)",
        },
        recursive: {
          type: "boolean",
          description: "Whether to search subdirectories recursively",
        ...
        include_warnings: {
          type: "boolean",
          description: "Whether to include warning-level fixes (button sizes, fonts)",
          default: false,
        },
        output_file: {
          type: "string",
          description: "Optional: Absolute path for JSON output file (e.g., 'C:\\temp\\ui-fix-plan.json')",
        },
      },
      required: ["source_dir"],
    },
  },
  {
    name: "apply_ui_fixes",
    description:
      "Apply UI fixes from JSON fix plan with comprehensive safety measures. Creates backups before modification, performs pre/post validation, detects corruption (mismatched braces, missing declarations, size changes >50%), and automatically rolls back on failure. Supports dry-run mode for validation before applying changes.",
    inputSchema: {
      type: "object",
      properties: {
        fix_plan_file: {
          type: "string",
          description: "Absolute path to JSON fix plan file (generated by generate_ui_fix_plan)",
        },
        backup_dir: {
          type: "string",
          description: "Optional: Absolute path for backup directory (default: auto-generated)",
        },
        dry_run: {
          type: "boolean",
          description: "If true, validate actions without applying changes",
          default: false,
        },
        max_severity: {
          type: "string",
          enum: ["critical", "error", "warning", "info"],
          description: "Maximum severity level to fix (default: warning)",
          default: "warning",
        },
      },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "validate_ui_scaling":
        return await validateUiScaling(
          args as { source_dir: string; recursive?: boolean; file_types?: string[] }
        );

      case "generate_ui_fix_plan":
        return await generateUiFixPlan(
          args as { source_dir: string; recursive?: boolean; include_warnings?: boolean; output_file?: string }
        );

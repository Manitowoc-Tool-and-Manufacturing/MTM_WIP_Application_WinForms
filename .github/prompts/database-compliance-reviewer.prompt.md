...
The MTM database standardization initiative defines 29 Functional Requirements (FR-001 to FR-029) and 18 Success Criteria (SC-001 to SC-018) extracted from 103+ specification files.

**PRIMARY REFERENCE**: `.github/references/database-compliance-fr-sc-reference.md`  
This consolidated reference file contains ALL database compliance requirements. **Read this file FIRST** - it replaces the need to read individual spec files.

The reference was generated by analyzing:
- specs/Archives/002-comprehensive-database-layer/ (Original draft)
- specs/Archives/002-003-database-layer-complete/ (Consolidated - 29 FRs, 18 SCs)
- specs/Archives/003-database-layer-refresh/ (Enhanced documentation)
- specs/Archives/test-fix-workspace/ (Test documentation)

**All requirements are mandatory and cumulative.** When reviewing code, validate against ALL FR/SC requirements from the reference file.
...
1. **Read Consolidated Requirements Reference** (CRITICAL - READ THIS FIRST):
   ```
   PRIMARY REFERENCE (Read First - Contains ALL Requirements):
   - Read .github/references/database-compliance-fr-sc-reference.md
   
   This single file contains:
   - All 29 Functional Requirements (FR-001 to FR-029) with full descriptions
   - All 18 Success Criteria (SC-001 to SC-018) with validation criteria
   - Quick reference matrix mapping FRs to SCs
   - Usage notes and standardized patterns
   - Extracted from 103+ specification files across all Archives
   
   SUPPLEMENTARY (Read Only If Needed for Clarifications):
   - specs/Archives/003-database-layer-refresh/clarification-questions.md
   - specs/Archives/003-database-layer-refresh/UPDATE-SUMMARY-SESSION-3.md
   - Quality checklists in specs/Archives/*/checklists/ directories
   ```
   Parse and internalize ALL FR-XXX (Functional Requirements) and SC-XXX (Success Criteria) from the reference file.
...
1. **Begin with requirements reference reading and MCP tool execution (Phase 1)**
   - Read .github/references/database-compliance-fr-sc-reference.md (contains ALL 29 FRs and 18 SCs)
   - Parse and internalize ALL FR-XXX and SC-XXX requirements from the consolidated reference
   - Understand the requirement categories and validation matrix
